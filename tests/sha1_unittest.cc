...
static const SHA1TestCase SHA1Tests[] = {
...
TEST_F(SHA1Test, Simple) {
  uint8_t digest[SHA1::DIGEST_SIZE];
  char hexdigest[SHA1::DIGEST_SIZE * 2 + 1];
  SHA1 sha1;

  for (size_t i = 0; i < arraysize(SHA1Tests); i++) {
    sha1.Init();
    sha1.Update(SHA1Tests[i].input, strlen(SHA1Tests[i].input));
    sha1.Final(digest);
    HexDump(hexdigest, digest, SHA1::DIGEST_SIZE);
    ASSERT_STREQ(SHA1Tests[i].digest, hexdigest);
  }
}

TEST_F(SHA1Test, ByteByByte) {
  uint8_t digest[SHA1::DIGEST_SIZE];
  char hexdigest[SHA1::DIGEST_SIZE * 2 + 1];
  SHA1 sha1;
  ...
  for (size_t i = 0; i < arraysize(SHA1Tests); i++) {
    sha1.Init();
    size_t length = strlen(SHA1Tests[i].input);
    for (size_t j = 0; j < length; j++)
      sha1.Update(&SHA1Tests[i].input[j], 1);
    sha1.Final(digest);
    HexDump(hexdigest, digest, SHA1::DIGEST_SIZE);
    ASSERT_STREQ(SHA1Tests[i].digest, hexdigest);
  }
