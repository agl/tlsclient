...
// These tests were taken from the Go test suite and recomputed for SHA256
// using Python's hashlib
static const SHA256TestCase SHA256Tests[] = {
...
{"c62e4615bd39e222572f3a1bf7c2132ea1e65b17ec805047bd6b2842c593493f", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"},
{"7ce100971f64e7001e8fe5a51973ecdfe1ced42befe7ee8d5fd6219506b5393c", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"},
};

struct HMACSHA256TestCase {
  const char* digest;
  const char* key;
  const char* input;
};

static const HMACSHA256TestCase HMACSHA256Tests[] = {
  {"b613679a0814d9ec772f95d778c35fc5ff1697c493715653c6c712144292c5ad", "", ""},
  {"5d5d139563c95b5967b9bd9a8c9b233a9dedb45072794cd232dc1b74832607d0", "key", ""},
  {"94b426611538beea003b4c250f864889eae6de3e620cb73e984e1c23ea5d7372", "keykey", "message"},
};

TEST_F(SHA256Test, Simple) {
  uint8_t digest[SHA256::DIGEST_SIZE];
  char hexdigest[SHA256::DIGEST_SIZE * 2 + 1];
  SHA256 sha256;

  for (size_t i = 0; i < arraysize(SHA256Tests); i++) {
    sha256.Init();
    sha256.Update(SHA256Tests[i].input, strlen(SHA256Tests[i].input));
    sha256.Final(digest);
    HexDump(hexdigest, digest, SHA256::DIGEST_SIZE);
    ASSERT_STREQ(SHA256Tests[i].digest, hexdigest);
  }
}

TEST_F(SHA256Test, ByteByByte) {
  uint8_t digest[SHA256::DIGEST_SIZE];
  char hexdigest[SHA256::DIGEST_SIZE * 2 + 1];
  SHA256 sha256;
  ...
  for (size_t i = 0; i < arraysize(SHA256Tests); i++) {
    sha256.Init();
    size_t length = strlen(SHA256Tests[i].input);
    for (size_t j = 0; j < length; j++)
      sha256.Update(&SHA256Tests[i].input[j], 1);
    sha256.Final(digest);
    HexDump(hexdigest, digest, SHA256::DIGEST_SIZE);
    ASSERT_STREQ(SHA256Tests[i].digest, hexdigest);
  }
...
typedef HMAC<SHA256> H;

TEST_F(SHA256Test, HMAC) {
  uint8_t digest[H::DIGEST_SIZE];
  char hexdigest[H::DIGEST_SIZE * 2 + 1];
  H hmac;

  for (size_t i = 0; i < arraysize(HMACSHA256Tests); i++) {
    hmac.Init(reinterpret_cast<const uint8_t*>(HMACSHA256Tests[i].key), strlen(HMACSHA256Tests[i].key));
    size_t length = strlen(HMACSHA256Tests[i].input);
    for (size_t j = 0; j < length; j++)
      hmac.Update(&HMACSHA256Tests[i].input[j], 1);
    hmac.Final(digest);
    HexDump(hexdigest, digest, H::DIGEST_SIZE);
    ASSERT_STREQ(HMACSHA256Tests[i].digest, hexdigest);
  }
}
